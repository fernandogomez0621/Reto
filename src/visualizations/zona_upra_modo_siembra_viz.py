...
def show_zona_upra_modo_siembra_dashboard(df):
    st.header("Distribución por Zona UPRA y Modo de Siembra")
    ...
    selected_zona = st.selectbox(
        ...
        zonas_orden,
        key="zona_upra_selectbox"
    )
    
    # Filtrar datos por zona seleccionada
    df_zona = df[df['Zona UPRA'] == selected_zona]
    
    # Contar frecuencias y calcular porcentajes por modo de siembra
    modo_counts = df_zona['Modo Siembra'].value_counts()
    ...
    fig.update_layout(
        ...
        showlegend=False,
        title={
            'text': f"<b>{selected_zona}</b>: Total de registros {total_modo:,}",
            'y': 0.95,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top'
    ...
    if st.checkbox('Mostrar datos detallados', key=f'show_data_details_{selected_zona}'):
